...
But a `Path` is immutable and doesn't **own** the path string data, only holds a reference of it. People seem to say it's [akin to `String` and `str`](https://doc.rust-lang.org/stable/std/path/index.html).
